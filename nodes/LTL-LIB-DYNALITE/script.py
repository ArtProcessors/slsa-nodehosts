# Philips Dynet Lighting Gateway -- local fixed version
#
# Based on the official recipe, with one change: every field that ends up in
# chr() is now normalised by asByte() before use. Without it the node dies with
#   TypeError: chr(): 1st arg can't be coerced to int
# whenever a value arrives as a string, a float or None -- see readme.md.
...
# args can arrive as str, float or None (dashboards, remote bindings, REST, blank
# schema fields), all of which blow up in chr(), so normalise to a 0-255 int here
def asByte(name, value, default=None):
  if value == None or value == '':
    if default == None:
      raise Exception('Dynalite field "%s" is missing' % name)
    value = default

  try:
    i = int(round(float(value)))
  except:
    raise Exception('Dynalite field "%s" is not a number: %r' % (name, value))

  if i < 0 or i > 255:
    raise Exception('Dynalite field "%s" out of range 0-255: %s' % (name, i))
...
def sendDynaliteMessage(packet=None):
  ...
  area = asByte('area', packet.get('area'))
  data0 = asByte('data0', packet.get('data0'))
  preset = asByte('preset', packet.get('preset'))
  if preset < 1:
    raise Exception('Dynalite field "preset" must be 1 or more (it is sent as preset-1)')
  data1 = asByte('data1', packet.get('data1'), 0)
  data2 = asByte('data2', packet.get('data2'), 0)
  join = asByte('join', packet.get('join'), 255)
