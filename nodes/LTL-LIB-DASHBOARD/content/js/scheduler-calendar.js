/*
 * Schedule calendar for the 'Calendar' tab, lifted from the LTL-LIB-SCHEDULER
 * node (content/js/scheduler-calendar.js). Two things differ from the original:
 *
 *  1. It mounts on .page[data-section="Calendar"] instead of that node's
 *     "Overview" page (index.xsl names each page div after its <page title>).
 *  2. Every REST call is prefixed with SCHEDULER_BASE and so talks to the
 *     LTL-LIB-SCHEDULER node directly, NOT to this dashboard. The calendar
 *     reads and writes that node's 'Schedules' PARAMETER, which - unlike the
 *     Media Player tab's actions and events - cannot be relayed through a
 *     local action stub, so there is nothing to wire in the Bindings editor.
 *
 * Loaded by js/custom.js (index.xsl honours only a single js= attribute on
 * <pages>); its own $(document).ready() handler still fires because jQuery
 * runs ready callbacks registered after the DOM is up immediately.
 */
...
(function ($, model) {
  ...
  // The page this script runs on is /nodes/xxDashboard/, so '../<node>/' walks
  // sideways to the scheduler node on the same host - same origin, no CORS.
  // Relative rather than '/nodes/...' so it survives the host being served
  // under a path prefix. Change SCHEDULER_NODE if the node is ever renamed;
  // node renames do not update anything that points at them.
  var SCHEDULER_NODE = 'LTL-LIB-SCHEDULER';
  var SCHEDULER_BASE = '../' + encodeURIComponent(SCHEDULER_NODE) + '/';

  // The tab this mounts in, i.e. <page title='Calendar'> in content/index.xml.
  var PAGE_SELECTOR = '.page[data-section="Calendar"]';
  ...
  function saveAndApply() {
    ...
    validateAllSchedules(saveSnapshot).done(function (failures) {
      ...
      $.getJSON(rest('REST/params')).done(function (currentParams) {
        ...
        $.postJSON(rest('REST/params/save'), JSON.stringify(payload)).done(function () {
          state.originalSchedules = clone(saveSnapshot);
          $.postJSON(rest('REST/actions/ApplySavedSchedules/call'), '{}').done(function () {
            state.saving = false;
  ...
  function bindEvents() {
    ...
    // On the scheduler node this was the only page, so the week view was always
    // visible when it first rendered. Here the dashboard opens on Overview and
    // this page is display:none, where the scroll container has no height and
    // scrollToWorkingHours() gives up - leaving the week parked at midnight.
    // Re-run it when the tab is shown. nodel.js's own '*[data-nav]' handler is
    // delegated off <body> and was bound first (it loads before custom.js), so
    // the page is already visible by the time this runs.
    $('body').on('click', '[data-nav="Calendar"]', function () {
      window.setTimeout(scrollToWorkingHours, 0);
    });
  }

  function loadSchedules() {
    $.getJSON(rest('REST/params')).done(function (params) {
