...
def recomputeMaster(ignoredArg=None):
  ...
  # a zone that has never reported is UNKNOWN, not agreeing -- claiming "in sync" or
  # "muted" on partial information could leave a zone audibly live while the dashboard
  # says otherwise, so those two only publish once every managed zone has been heard from
  allGainsKnown = len(gains) == len(_masterZones)
  allMutesKnown = len(mutes) == len(_masterZones)

  if len(gains) > 0:
    # report the loudest zone so the fader still shows a meaningful (and safe-side)
